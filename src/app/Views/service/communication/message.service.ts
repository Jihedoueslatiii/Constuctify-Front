...
// Update hate speech flag in the database
updateHateSpeechStatus(messageId: string, isHateSpeech: number): Observable<any> {
  const body = {
    messageId: messageId,
    isHateSpeech: isHateSpeech
  };

  return this.http.post<any>('http://localhost:5000/update-hate-speech', body);
}
  ...
  getPrediction(content: string): Observable<any> {
    ...
    console.log('Sending prediction body:', body);
  ...
  sendMessage(conversationId: number, content: string, isHateSpeech: string, mediaUrl?: string, isPM: boolean = false): Observable<Message> {
    const currentUser = this.authService.getUserInfo();
    const currentUserId = currentUser?.id; // Access ID from the userInfo

    const body = new HttpParams()
      .set('senderId', currentUserId ? currentUserId.toString() : '1') // Use currentUserId if available, otherwise fallback (though this should ideally not happen)
      .set('conversationId', conversationId.toString())
      .set('content', content)
      .set('isHateSpeech', isHateSpeech)
      .set('isPM', isPM.toString())
    ...
    console.log('isHateSpeech:', isHateSpeech);
