...
ngOnInit(): void {
  ...
  const userId = this.authService.getCurrentUser(); // or wherever you store it
    ...
    this.messageService.getPrediction(content).subscribe({
      next: (result) => {
        if (result.isHateSpeech) {
          // If it's hate speech, show a warning and don't proceed
          console.warn('Message contains hate speech and will not be sent.');
          return;
        }

        // Step 2: Send the message if it's safe (no hate speech)
        this.messageService.sendMessage(conversationId, content).subscribe({
          next: () => {
            this.newMessage = ''; // Clear input field
            this.refreshMessages(); // Refresh messages list
          },
          error: (err) => console.error('Failed to send message:', err)
        });
      },
      error: (err) => console.error('Prediction check failed:', err)
    });
  }
